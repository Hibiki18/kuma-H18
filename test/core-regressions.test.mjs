...
test('受损语音弹幕按播放时刻血量分四档，Toast 按配置只落一个窗口', () => {
  ...
  // 独立游戏宿主覆盖工作台的游戏容器：选游戏画面时只走宿主 IPC，选 kuma 窗口
  // 或宿主尚未布局时只在工作台创建 DOM，不能把同一条卡同时画两份。
  ...
  assert.match(bell, /if \(publishToastToGame\(def, title, detail, ref\)\) return/)
  assert.match(bell, /publishGameToast\(/)
  assert.match(bell, /wrapper && wrapper\.clientWidth > 0 \? position : null/)
  ...
  assert.match(hostHtml, /#lg-toasts\[data-corner\$="r"\] \{ right: 12px; \}/)
  assert.match(hostHtml, /width: min\(250px, calc\(100% - 24px\)\)/)
})
