...
test('弹卡落点:一个键都没存过时,还是可配之前那个位置', () => {
  ...
  assert.equal(lg.toasts().length, 0, '游戏宿主与工作台同时画了同一张卡')
  assert.equal(lg.overlayEvents().length, 1, '默认没发进游戏宿主')
  assert.equal(lg.overlayEvents()[0].corner, 'br', '默认不是右下角')
})
...
test('弹卡落点:四个角逐个都认,参照系不跟着变', () => {
  for (const corner of TOAST_CORNERS) {
    ...
    lg.show('第1舰队有中破', '旗舰中破', 1, false)
    assert.equal(lg.overlayEvents()[0]?.corner, corner, `${corner} 这一角没发给宿主`)
    assert.equal(lg.boxHost(), null, `${corner} 同时在工作台复制了一张`)
  }
})
...
test('弹卡落点:从游戏画面改到 kuma 窗口后,新卡只落工作台', () => {
  const lg = mountLgToast({ gameWrapper: true })
  lg.show('第1舰队有中破', '旗舰中破', 1, false)
  assert.equal(lg.overlayEvents().length, 1)
  assert.equal(lg.boxHost(), null)
  ...
  assert.equal(lg.boxHost(), 'body', '换了参照系后新卡没落工作台')
  assert.equal(lg.boxCorner(), 'bl')
  assert.equal(lg.toasts().length, 1, '换档后的单条通知在工作台重复出现')
  assert.equal(lg.overlayEvents().length, 1, '换到工作台后仍向游戏宿主复制通知')
})
...
test('弹卡落点:还没实装的参照系与乱值都退回默认,弹卡永远有归宿', () => {
  ...
  lg.show('第1舰队有中破', '旗舰中破', 1, false)
  assert.equal(lg.overlayEvents().length, 1, '存了个做不出来的落点,弹卡就没了')
  assert.equal(lg.overlayEvents()[0].corner, 'br')
})
...
test('弹卡落点:四个角的偏移在 CSS 里都摆得出来', () => {
  ...
  const ruleIn = (source, selector) =>
    new RegExp(`${selector.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\s*\\{[^}]*\\}`).exec(source)?.[0]
  const rule = (selector) => ruleIn(html, selector)
  ...
  assert.match(ruleIn(hostHtml, '#lg-toasts[data-corner^="t"]') ?? '', /top: 12px/, '游戏宿主的上两角没贴边')
