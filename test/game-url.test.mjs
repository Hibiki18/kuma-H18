...
test('游戏 webview 的 src 过判据，不是把配置原样递进去', () => {
  const manager = read('src/main/game-host-manager.ts')
  const host = read('src/renderer/game-host.ts')
  assert.match(
    manager,
    /homepage: normalizeGameUrl\(config\.get\(GAME_URL_CONFIG_KEY\)\)/,
    '装 webview 时没过 normalizeGameUrl——写坏的配置会直接变成白屏',
  )
  assert.match(host, /view\.src = bootstrap\.homepage/)
  // 默认值不许在别处再写一遍：两份字面量迟早会各走各的
  for (const rel of ['src/main/game-host-manager.ts', 'src/renderer/game-host.ts', 'src/main/config.ts']) {
    assert.ok(
...
test('生效那条路是这张卡自己的按钮，不是顶栏那个刷新', () => {
  ...
  assert.match(
    ...
    /#btn-reload'\)\.addEventListener\('click', \(\) => \{\s*void executeGameCommand\(\{ type: 'reload' \}\)/,
  )
})
