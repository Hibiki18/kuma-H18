...
//      只在 `KANSO_DEBUG_UI=1` 下装配（发行版 21 张 / 调试 23 张）。
//      玩家那份产物里连那两张卡的影子都不许有。
...
test('分类表：抽掉维护者工具卡之后，没有一类被掏空或只剩一张', () => {
  ...
  // 21：2026-08-29 给魔改玩家铺路时添了「魔改文件夹」（诊断类，紧跟缓存修复），
  // 发行版那一列第一次从 18 变到 19；2026-08-30 又添了「游戏页面网址」
  //（网络类，紧跟代理）到 20，同日的游戏画面缩放两档添了「游戏画面」
  //（界面类，紧跟界面缩放）到 21。三张都是**玩家卡**，不在调试门后。
  assert.equal(release.length, 21, '发行版的卡数变了——两种形态的数字都要重新对一遍')
  // 23：2026-08-26 拔掉战斗演出族时撤走了「索敌飞机 · Δ 校准」那张维护者卡（19），
  // 同日修语音滑条时又添了「游戏音频链路自检」（20）。那两张都只在调试门后装配，
  // 所以发行版那一列在 2026-08-29 之前自始至终是 18；之后添的三张玩家卡两列同涨。
  assert.equal(SETTINGS_CARD_IDS.length, 23, '调试态的卡数变了')
})
