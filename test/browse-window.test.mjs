...
test('UA 清洗只有一份判据：游戏页与浏览窗都引它，谁也别再自己写正则', () => {
  ...
  const gameHost = code('src/main/game-host-manager.ts')
  const browse = code('src/renderer/browse-window.ts')
  assert.match(gameHost, /cleanUserAgent\(this\.mainWindow\.webContents\.userAgent\)/)
  assert.match(browse, /cleanUserAgent\(navigator\.userAgent\)/)
  assert.doesNotMatch(gameHost, /userAgent[\s\S]{0,120}\.replace\(/, '游戏宿主又在本地写了一份 UA 清洗')
  assert.doesNotMatch(browse, /navigator\.userAgent\s*\.?\s*\n?\s*\.replace/, '浏览窗又在本地写了一份 UA 清洗')
})
...
test('游戏 webContents 的认定只有一处，浏览窗进不去那本册子', () => {
  // 认定 = 游戏宿主 did-attach-webview 里记 id；kcs-resource 的记账与入档全按这个 id 过闸。
